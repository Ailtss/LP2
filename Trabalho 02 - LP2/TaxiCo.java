...
//        shuttleFleet = new ArrayList<Shuttle>();
    ...
    private void fillDestinations()
    {
		...
		/**
		 * Método criado para satisfazer o Desafio proposto pelo Trabalho 2 de LP2.
		 */

		public Vehicle calling(String destination)
		{
			int i;
			for(i = 0; i < vehicleFleet.size(); i++) //Primeiro testando todos os shuttles da lista devido a sua prioridade.
			{
				if(vehicleFleet.get(i) instanceof Shuttle)
					{
						if(vehicleFleet.get(i).getDestination() == destination)
							{
								return vehicleFleet.get(i);
							}
					}
			}

			for(i = 0; i < vehicleFleet.size(); i++) //Agora verificando se há algum taxi livre. Retorna o primeiro livre.
			{
				if(vehicleFleet.get(i) instanceof Taxi)
					{
						if(vehicleFleet.get(i).getDestination() == null)
						  {	
								return vehicleFleet.get(i);
							}
					}
			...
			return null; // Retorna null se não houver nenhum veículo disponível.
		}
}
